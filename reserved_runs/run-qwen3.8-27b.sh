#!/bin/bash
# Qwen3.8-27B-FP8 서빙 (27GB, dense, vision-language, 2노드 클러스터: raven+quaker, tp2)
# NVFP4 대신 FP8 선택 — 품질이 원본 bf16과 거의 동일하다고 Qwen이 명시.
# 작아서 굳이 tp2가 필요친 않지만 dense라 compute-bound라 디코드 속도 이득 볼 수도 있음.
# 메모리만 필요하면 solo가 나음: run-qwen3.8-27b-solo.sh
# 네트워크가 병목이면 pp2가 나을 수도: run-qwen3.8-27b-pp2.sh
...
exec ./run-recipe.sh qwen3.8-27b-fp8 --env HF_HUB_OFFLINE=1 --env TRANSFORMERS_OFFLINE=1 "$@"
