#!/bin/bash
# Qwen3.6-27B-NVFP4 서빙 (~14GB, dense, 2노드 클러스터: raven+quaker, tp2)
# FP8 대비 절반 크기, 디코드 빠름. 품질은 FP8보다 소폭 하락: run-qwen3.6-27b.sh
# 작아서 굳이 tp2가 필요친 않지만 dense라 compute-bound라 디코드 속도 이득 볼 수도 있음.
# 메모리만 필요하면 solo가 나음: run-qwen3.6-27b-nvfp4-solo.sh
# 네트워크가 병목이면 pp2가 나을 수도: run-qwen3.6-27b-nvfp4-pp2.sh
